...
Hence, taking everything into account, HRF also outperforms CRF, on balance.\footnote{Appendix \ref{sec:results-in-table-format} presents the RMSE results for all datasets and all methods considered (RF, HRF, WRF, CRF, Ridge, 2-Step Ridge, and HRF-MinVar).}

\textcolor{blue}{%
We further benchmark HRF against three combination schemes that are not
...
with the ridge penalty chosen by AIC. \emph{2-Step Ridge} uses the same
estimator but, following the sample-splitting idea of
\cite{timmermann:2006}, estimates $w$ on a held-out second half of
the training set rather than on the sample used to grow the trees, so as
to avoid in-sample overfitting. \emph{HRF-MinVar} solves the same optimization
...
balance, at every training-set size. 2-Step Ridge, in contrast, becomes
highly unstable whenever the training-set size is close to twice the
number of trees $p = 500$ (so that the second-stage regression is
estimated from about as many observations as there are trees to weight),
causing its RMSE to spike relative to HRF; this instability is most
pronounced at $n=1000$ and illustrates the benefit of estimating the
covariance matrix $\Sigma$ with nonlinear shrinkage on the full training
sample, as HRF does, rather than resorting to sample splitting.
}
...
As an additional application to finance, we consider the ``Black Friday Sales Prediction" challenge. The dataset is available at
\url{www.kaggle.com/datasets/sdolezel/black-friday} and further information can be found at \url{www.analyticsvidhya.com/datahack/contest/black-friday}.
The goal is to forecast the sales amount for a specific purchase on Black Friday based on 11 features, such as gender, age, occupation, and marital status of the purchaser, as well as city and product categories. The total number of observations in the dataset is over half a million, so we select 10,000 observations at random as in Section~\ref{sec:data}. Then we compute RMSE ratios \eqref{eq:rmse-rat}for various training-set sizes $n$ as described in Section~\ref{sec:results}.  Table~\ref{table:friday} presents the results which demonstrate that HRF outperforms RF consistently for all training-set sizes $n$.
...
5000 & 0.977 \\
   \bottomrule
\end{tabular}
\caption{RMSE ratios \eqref{eq:rmse-rat} for various training-set sizes $n$.} 
\label{table:friday}
\end{table}
...
standard random forest. The evidence
...
Things change in the case of time-series data: What are suitable estimators of $\mu$  and~$\Sigma$ depends on the nature and frequency of the data and on the sample size. Therefore, these estimators have to be custom-tailored to the application at hand. This is unfortunate but also ``par for the course" for time-series applications where ``one-size-fits-all" solutions rarely exist.

\cite{beck:wolf:hrf-inf} detail how to adapt the hedged random forest to the specific task of forecasting inflation and demonstrate that it consistently outperforms the standard random forest in terms of forecasting accuracy.
...
This appendix presents the RMSE results for all datasets, training-set sizes, and forecasting methods discussed in Sections~\ref{sec:results} and \ref{sec:comp1}\textcolor{blue}{, including the Ridge, 2-Step Ridge, and HRF-MinVar benchmarks of Section~\ref{sec:comp1}}.
\label{sec:results-table-format}
\input{tables/weighted_rf_rmse_200}
\input{tables/weighted_rf_rmse_400}
\input{tables/weighted_rf_rmse_600}
\input{tables/weighted_rf_rmse_800}
\input{tables/weighted_rf_rmse_1000}
\input{tables/weighted_rf_rmse_2000}
\input{tables/weighted_rf_rmse_3000}
\input{tables/weighted_rf_rmse_4000}
\input{tables/weighted_rf_rmse_5000}
