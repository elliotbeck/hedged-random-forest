...
\textcolor{blue}{Unless stated otherwise, from this point on we use the term HRF to
refer specifically to this baseline specification, that is, nonlinear
shrinkage estimation of $\Sigma$ combined with $\kappa = 2$. Whenever we
depart from this default---for example, by using the sample covariance
matrix instead of nonlinear shrinkage, or a value of $\kappa$ other than
$2$---we state so explicitly.}
...
\input{tables/fredmd_rmse_ratios}
}
